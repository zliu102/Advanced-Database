...
RC createPageFile(char* fileName) {
	FILE *fp = NULL;
	fp = fopen(fileName, "w+"); //Open file for read and write(overwrite existing one)
    if(fp == NULL) {
    ...
    size_t w = fwrite(emptyPage, 1, PAGE_SIZE, fp);
    free(emptyPage);
    if( w != PAGE_SIZE){
...
RC openPageFile (char *fileName, SM_FileHandle *fHandle){
	FILE *fp = NULL;
	fp = fopen(fileName, "r+"); //Open file for write and read
    if(fp == NULL) {
        THROW(RC_FILE_NOT_FOUND, "File not found");
    }

    fHandle->fileName = fileName;   // Record the filename
    if (fseek(fp, 0, SEEK_END) != 0){ //Move pointer to the end
        fclose(fp);
        THROW(RC_GET_PAGE_SIZE_FAILED, "Failed to get page size");
    };

    fHandle->totalNumPages = (int)(ftell(fp) / PAGE_SIZE); //Get total number of pages
    fseek(fp, 0, SEEK_SET); //Move pointer back to the beginning
    fHandle->curPagePos = 0;       // Set the current position to the beginning of file
    fHandle->mgmtInfo = fp;  // POSIX file descriptor
//    fclose(fp);
